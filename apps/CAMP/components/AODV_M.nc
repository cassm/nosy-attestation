...
implementation {
    ...
    bool pending_rreq[MAX_NETWORK_SIZE];
	...
	for(i = 0; i< AODV_ROUTE_TABLE_SIZE; i++) {
	    ...
	    route_table_[i].dest = 255;
	    route_table_[i].next = 255;
	    route_table_[i].hop  = 0;
	}
    
	for(i = 0; i< AODV_RREQ_CACHE_SIZE; i++) {
	    rreq_cache_[i].seq  = 0;
	    rreq_cache_[i].dest = 255;
	    rreq_cache_[i].src  = 255;
	    rreq_cache_[i].hop  = 0;
	}
    
	for(i = 0; i< MAX_NETWORK_SIZE; i++) {
	    pending_rreq[i] = FALSE;
	}
	...
	if( forward == FALSE ) { // generate the RREQ for the first time
	    ...
	    // only have one request for each dest at once
	    if (pending_rreq[dest])
		return TRUE;

	    pending_rreq[dest] = TRUE;
	...
	printf("AODV: add_route_table() seq:%d dest:%d next:%d hop:%d\n",
    ...
    //--------------------------------------------------------------------------
    //  Routefinder.getNextHop. If a known route exists, signal immediately.
    //  Otherwise, send an rreq and signal on the first appropriate rrep.
    //--------------------------------------------------------------------------
    command error_t RouteFinder.getNextHop( uint8_t dest ) {
	uint8_t nextHop;

	nextHop = get_next_hop( dest );

	// if address is this node
	if ( dest == TOS_NODE_ID ) {
	    signal RouteFinder.nextHopFound(TOS_NODE_ID, dest);
	    return SUCCESS;
	}
      
	// if no link is yet known
	else if ( nextHop == 255 ) {	  
	    // send query, wait for reply
	    sendRREQ( dest, FALSE );
	}
      
	// if next hop is known
	else {
	    signal RouteFinder.nextHopFound(nextHop, dest);
	}
	return SUCCESS;
    }    
	...
	if( aodv_hdr->src == call AMPacket.address() ) {
	    ...
	    // if pending rreq, signal route
	    if (pending_rreq[aodv_hdr->dest]) {
		pending_rreq[aodv_hdr->dest] = FALSE;
		signal RouteFinder.nextHopFound(src, aodv_hdr->dest);
	    }
	}
	else { // not to me
