...
module AODV_M {
  ...
  uses {
    ...
    interface MsgQueue as QueryQueue;
    interface MsgQueue as WaitingQueue;
  }
}
...
implementation {
  ...
  command error_t RouteFinder.getNextHop(message_t *msg) {
      if ( call QueryQueue.push(msg) != SUCCESS )
	  return ENOMEM;
      post queryTask();
      return SUCCESS;
  }
  ...
  task void queryTask() {
      if ( queryBusy || call QueryQueue.isEmpty() ) 
	  return;
      queryBusy = TRUE;
      ...
      queryBuffer = *call QueryQueue.pop();
      addr = (checksummed_msg_t*)(queryBuffer.data)->dest;
      nextHop = get_next_hop( addr );

      // if address is this node
      if ( addr == TOS_NODE_ID ) {
	  (checksummed_msg_t*)(queryBuffer.data)->next = addr;
	  signal RouteFinder.nextHopFound(&exitBuffer);
	  queryBusy = FALSE;
	  post queryTask();
      }
      
      // if no link is yet known
      else if ( nexthop == INVALID_NODE_ID ) {	  
	  ...
	  (checksummed_msg_t*)(queryBuffer.data)->next = nextHop;
      
	  exitBuffer = queryBuffer;
	  signal RouteFinder.nextHopFound(&exitBuffer);
	  queryBusy = FALSE;
	  post queryTask();
      }
